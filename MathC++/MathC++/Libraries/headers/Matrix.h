...
public:
	...
	Matrix(const initializer_list<LinearEquation<T>>&);//1
	static Matrix<T> E(size_t);//1

	size_t size() const;//1
	virtual ostream& print(ostream&) const;
	//1
	T determinant() const;//1
	Matrix<T> transposed() const;//1
	Matrix<T> inversed() const;//1
	Matrix<T> power(int) const;//1

	LinearEquation<T>& operator[](size_t);//1
	const LinearEquation<T>& operator[](size_t)const;//1
	template<class F>
	friend Matrix<F> operator*(const Matrix<F>&, const F&);//0
	template<class F>
	friend Matrix<F> operator*(const F&, const Matrix<F>&);//0
	template<class F>
	friend Matrix<F> operator+(const Matrix<F>&, const Matrix<F>&);//1
	template<class F>
	friend Matrix<F> operator-(const Matrix<F>&, const Matrix<F>&);//1
	template<class F>
	friend Matrix<F> operator*(const Matrix<F>&, const Matrix<F>&);//1
	
	Matrix<T> operator+() const;//1
	Matrix<T> operator-() const;//1

	template<class F>
	friend bool operator==(const Matrix<F>&, const Matrix<F>&);//1
	template<class F>
	friend bool operator!=(const Matrix<F>&, const Matrix<F>&);//1
...
Matrix<T> operator+(const Matrix<T>& left, const Matrix<T>& right) {
	...
	Matrix<T> m(vector<LinearEquation<T>>(left.size(), LinearEquation<T>(left.size())));
	for (size_t i = 0; i < left.size(); ++i)
		m[i] = left[i] + right[i];
	return m;
}
